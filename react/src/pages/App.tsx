...
const DebugCoordsLayerComponent = createLayerComponent(
  DebugCoordsLayer /* , DebugCoordsLayerUpdate */
);
    ...
    <>
      ...
      >
        ...
        {holeSate.pointOnMap.map((point) => {
          const { /*  direction, */ distance } = getDirectionAndAngle(
            holeSate.userLocation as L.LatLng,
            point.coord as L.LatLng,
            point.missdValue as { lat: number; lng: number }
          );
          return (
            <Polyline
              key={point.id}
        ...
        {holeSate.connectedPoints.map((point) => {
          const { /*  direction, */ distance } = getDirectionAndAngle(
            point.startPoint.coord as L.LatLng,
            point.endPoint.coord as L.LatLng
          );
          return (
            <Polyline
        ...
        <ImageOverlay
          ...
          bounds={[
            [
              (center?.lat as number) - 0.5 / 10,
              (center?.lng as number) - 0.5 / 10,
            ],
            [
              (center?.lat as number) + 0.5 / 10,
              (center?.lng as number) + 0.5 / 10,
